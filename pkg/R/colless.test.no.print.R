...
colless.test.no.print <- function(tree, model = "yule", alternative = "less", n.mc = 500) {
    colless3 <- function(n, model, p = 1/3) {
        if (n > 2) {
            l <- switch(model, biased = sample(n - 1, 1, 0.5 * dbinom(0:(n - 2), size = (n - 2), prob = p) + 0.5 * dbinom(0:(n - 2), size = (n - 2), prob = (1 - p))), 
                yule = sample(n - 1, 1), aldous = sample(n - 1, 1, prob = (1/(1:(n - 1))/((n - 1):1))))
    ...
    # cat('\tTest of the ') cat(model) cat(' hypothesis using the Colless index ', '\n') cat('Statistic = ') cat(stat, '\n') cat('Standardized Statistic = ')
    # cat(stat_norm, '\n') cat('p-value = ')
    if (alternative == "less") {
        p.value <- res(stat)
        # cat(p.value, '\n') cat('alternative hypothesis: the tree is more balanced than predicted by the ') cat(model) cat(' model', '\n')
    } else if (alternative == "greater") {
        p.value <- 1 - res(stat)
        # cat(p.value, '\n') cat('alternative hypothesis: the tree is less balanced than predicted by the ') cat(model) cat(' model', '\n')
    } else {
        # cat('\n')
        stop("Argument 'alternative' is incorrect. Should be a string 'less' or 'greater'")
    }
    # cat('\n') cat('Note : the p-value was computed using a Monte-Carlo method') cat('\n')
