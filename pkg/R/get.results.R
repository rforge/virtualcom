		...
		list.of.results <- lapply(1:length(output), function(x) {
			...
			if(length(allInv)>=1){ 
				for(inv in 1:length(allInv)){
					temp <- data.frame(c(output[[x]]$parameter, output[[x]]$pool$indices), eval(parse(text=paste("output[[",x,"]]$invaders$indices[[",inv,"]]$", myvar, sep=""))), allInv[inv])
				    if(inv==1) tempA <- temp else tempA <- rbind(tempA,temp)
				}			
			if(nrow(tempA) >= 2){
				return(tempA)
			}}})	
