...
moving.window<-function(com_land, cell){
#cell=2
...
for (i in 1:side2)  {
  for (j in 1:side2)  {
      agg_land[which(agg_land$X==i & agg_land$Y==j),c(4:103)] <-  colSums(com_land[which(com_land$X %in% c(i:(i+(cell-1))) & com_land$Y %in% c(j:(j+(cell-1)))),4:103])
      agg_land[which(agg_land$X==i & agg_land$Y==j),"env"] <-  mean(com_land[which(com_land$X %in% c(i:(i+(cell-1))) & com_land$Y %in% c(j:(j+(cell-1)))),"env"])
      }
    }
#head(agg_land)
return(agg_land)
